...
class Steering:
    ...
    def __experimentallyFindMaxLeftPosition(self, __steering_speed):
        ...
        for i in range(0, INITIALIZE_EXPERIMENTAL_TURN_MAX_CYCLE_COUNT):
            self.steering_motor.run_to_rel_pos(position_sp=INITIALIZE_EXPERIMENTAL_TURN_POSITION_SP, speed_sp=__steering_speed, stop_action='hold')
    ...
    def initialize(self):
        ...
        self.__experimentallyFindMaxLeftPosition(__steering_speed)
        logging.debug('initialize: steering motor position after experimentally turning max left = '+ str(self.steering_motor.position))
        self.steering_motor.run_to_rel_pos(position_sp=-(self.steering_max_range + INITIALIZE_RETURN_TO_CENTER_COMPENSATE_POSITION_SP), speed_sp=__steering_speed, stop_action='hold')
        time.sleep(1)
    ...
    def getCurrentSteeringPosition(self):
        ...
        __current_steering_position = (self.steering_motor.position * 100 / self.steering_max_range)
        __rounded_current_steering_position = round(__current_steering_position, STEERING_POSITION_ROUND_DECIMAL_PLACES)
        logging.debug('getCurrentSteeringPosition: rounded steering position = '+ str(__rounded_current_steering_position))
        return __rounded_current_steering_position
